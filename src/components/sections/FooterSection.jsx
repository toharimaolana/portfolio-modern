...
const FooterSection = () => {
  ...
  return (
    <footer className="relative w-full bg-bg-base pt-16 sm:pt-20 pb-12 overflow-hidden border-t border-border-highlight/10">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1200px] px-6">
        ...
        {/* Asymmetric Split Columns — Responsive Layout */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-12 pb-14 md:pb-16 items-start">

          {/* LEFT COLUMN: Let's Connect CTA */}
          <div className="md:col-span-7 space-y-3 sm:space-y-4">
            <span className="font-mono text-[0.68rem] sm:text-xs tracking-[0.25em] text-accent-glow uppercase block">
              // COLLABORATION INQUIRIES
            </span>
            <h3 className="font-poetsen text-3xl sm:text-4xl lg:text-5xl font-black tracking-[-0.05em] uppercase text-text-light leading-tight sm:leading-none">
              Let&apos;s Connect
            </h3>
          ...
          {/* RIGHT COLUMN: Minimalism Monospace Socials & Dashboard Likes */}
          <div className="md:col-span-5 flex flex-col items-start md:items-end space-y-6 sm:space-y-8 w-full">

            {/* Social Text links in JetBrains Mono */}
            <div className="flex flex-col items-start md:items-end gap-3 w-full">
              <span className="font-mono text-[0.68rem] tracking-[0.2em] text-text-muted uppercase">
                // CONNECT ABROAD
              </span>
              <div className="flex flex-wrap items-center gap-x-4 sm:gap-x-5 gap-y-2">
                {[
                  ...
                  <a
                    ...
                    className="font-mono text-xs sm:text-xs tracking-[0.15em] text-text-muted hover:text-accent-glow transition-colors duration-300 relative group py-1"
                  >
                    {social.label}
                    <span className="absolute left-0 bottom-0 w-0 h-px bg-accent-glow transition-all duration-300 group-hover:w-full" />
                  </a>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col items-start md:items-end gap-2.5 w-full">
              ...
              <button
                ...
                className="
                  ...
                  rounded-xl px-4 sm:px-5 py-3 text-xs font-mono tracking-widest text-text-light/85 
        ...
        {/* BRAND SIGNATURE WATERMARK — Responsive Text Scaling without Horizontal Scroll */}
        <div className="w-full text-center overflow-hidden py-8 sm:py-10 pointer-events-none select-none">
          <h2 className="font-poetsen text-[6.5vw] sm:text-[7.5vw] font-black tracking-[-0.05em] leading-none uppercase text-text-light/[0.02] bg-gradient-to-b from-text-light/[0.03] to-transparent bg-clip-text text-transparent whitespace-nowrap">
            MOHAMAD TOHARI MAOLANA
          </h2>
        </div>

        {/* BOTTOM METADATA ROW */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left font-mono text-[0.68rem] tracking-wider text-text-muted/60 uppercase">
          <div>
      ...
      <AnimatePresence>
        {showScroll && (
          <motion.button
            onClick={scrollToTop}
            className="
              fixed bottom-6 right-6 sm:bottom-8 sm:right-8 z-50 rounded-full border border-border-highlight/30
              bg-bg-surface/80 p-3 sm:p-3.5 text-accent-glow shadow-[0_4px_24px_rgba(0,0,0,0.5)]
